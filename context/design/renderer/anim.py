...
from wordmark2 import mark_reveal
...
def page(e, f=FIX):
    ...
    if tk >= 1:
        ...
        cells_text(cv, '%02d' % f['h'], 'bold', BIG, 'WHITE', X0, HOURS_BASE, tk)
    if e['icon_fade'] > 0:
    ...
    if tk >= 1:
        ...
        cells_text(cv, '%02d' % f['m'], 'bold', BIG, 'BLACK', X0, MIN_BASE, tk)
        cells_text(cv, '%02d' % f['s'], 'bold', SEC_PX, 'BLACK', SEC_PEN, MIN_BASE, tk)
    dk = e.get('date', 1.0)
    if dk >= 1:
        cv.text(f['date'], 'mono', 23, 'WHITE', cx=233, y=334)
    else:
        cells_text(cv, f['date'], 'mono', 23, 'WHITE', pen_for(f['date'], 'mono', 23, cx=233),
                   334 + cap_height('mono', 23), dk)
    if e.get('fade_mode'):
...
# ---- 30 fps (round 3 onward): timing in frames ------------------------------------------------------
FPS = 30
FRAME_MS = 1000 / FPS


def kf(n, start, count):
    """Share of a stepped reveal shown at frame n: `count` steps on frames start .. start+count-1."""
    return clamp((n - start + 1) / count)


def save_gif30(frames, path, slow=1):
    """GIF delays are in 10 ms units: 30, 30, 40 ms averages 33.3 ms (times `slow`)."""
    pal = [f.convert('P', palette=Image.ADAPTIVE, colors=64) for f in frames]
    dur = [(40 if i % 3 == 2 else 30) * slow for i in range(len(pal))]
    pal[0].save(path, save_all=True, append_images=pal[1:], duration=dur, loop=0, disposal=1, optimize=False)


def entry_frames(opt, n):
    """The clock face's entry (option B) restated in frames at 30 fps: same order and about the
    same durations as the ms version, every step on a frame. Ends at frame 13 (433 ms)."""
    if n < 0:
        return entry(opt, -1)
    return dict(ring=kf(n, 0, 4), icon_fade=1.0, icon_rows=int(kf(n, 2, 5) * 5),
                label=kf(n, 3, 4), plate=kf(n, 5, 4), zone=kf(n, 7, 5), mark=kf(n, 9, 5))
