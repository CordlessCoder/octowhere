...
def cells_text(cv, s, face, px, col, pen_x, base, k=1.0, block=None):
    """Text on its character cells. k in 0..1 reveals left to right: each cell shows a
    solid block for one step before its glyph. Blocks never show a wrong character, only
    where one will be. Cells follow each glyph's own advance, so proportional faces work."""
